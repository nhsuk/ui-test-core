...
class BasePage:
    ...
    """
    def __init__(self, driver, logger=None, wait_time=10):
        """
        Default constructor which passes the control of webDriver to the current page
        :param driver: the Selenium web driver
        :param logger: logger object used to save information to a log file, None by default
        ...
        self.wait = Waiter(driver, self.find, wait_time, logger)
        self.interrogate = Interrogator(driver, self.find, logger)
        self.interact = Interactor(driver, self.find, self.interrogate, self.wait, logger)
